...
#define EPSILON 1e-9
#define pairdd std::pair<double, double>
...
{
	const int n = 4, m = 1;
	std::vector<std::pair<double, double>> first_set, second_set;
	...
	for (int i = 0; i < n; i++)
	{
		double x, y;
		std::cin >> x >> y;
		first_set.push_back(pairdd(x, y));
	}
	...
	for (int i = 0; i < m; i++)
	{
		double x, y;
		std::cin >> x >> y;
		second_set.push_back(pairdd(x, y));
	}
	...
	for (int i = 0; i < n - 3; i++)
		for (int j = i + 1; j < n - 2; j++)
			for (int k = j + 1; j < n - 1; j++)
				...
				{
					inside_square = true;
					if (is_square(first_set[i].first, first_set[i].second, first_set[j].first, first_set[j].second,
						first_set[k].first, first_set[k].second, first_set[h].first, first_set[h].second))
					{
						for (int l = 0; l < m; l++)
							if (!in_square(first_set[i].first, first_set[i].second, first_set[j].first, first_set[j].second,
								first_set[k].first, first_set[k].second, first_set[h].first, first_set[h].second, second_set[l].first, second_set[l].second))
							{
								inside_square = false;
								break;
							}
						if (inside_square)
						{
							double area = square_area(dist(first_set[i].first, first_set[i].second, first_set[j].first, first_set[j].second),
								dist(first_set[i].first, first_set[i].second, first_set[k].first, first_set[k].second));
							if (area < min_area || min_area < 0)
							{
								min_area = area;
								x1 = first_set[i].first; y1 = first_set[i].second;
								x2 = first_set[j].first; y2 = first_set[j].second;
								x3 = first_set[k].first; y3 = first_set[k].second;
								x4 = first_set[h].first; y4 = first_set[h].second;
							}
						}
					}
				}
