...
{
	flight_req* info;
	req_tree* left_subtree;
	req_tree* right_subtree;

	req_tree(flight_req* info)
	{
		this->info = info;
		this->left_subtree = new req_tree();
		this->right_subtree = new req_tree();
	}

	req_tree()
	{
		this->info = new flight_req();
	...
	void add_req(flight_req* info)
	{
		if (this->info->flight_num <= info->flight_num)
			if (this->right_subtree->info->flight_num == 0)
			{
				this->right_subtree = new req_tree(new flight_req());
			...
			if (this->left_subtree->info->flight_num == 0)
			{
				this->left_subtree = new req_tree(new flight_req());
	...
	{
		if (this->left_subtree->info->flight_num == 0 &&
			this->right_subtree->info->flight_num == 0)
		{
			delete(this->left_subtree);
			delete(this->right_subtree);
			delete(this->info);
			this->info = new flight_req();
			this->left_subtree = nullptr;
			this->right_subtree = nullptr;
		...
		if (this->left_subtree->info->flight_num == 0)
		{
			req_tree* temp = this->right_subtree;
			this->info = temp->info;
			...
			if (this->right_subtree->info->flight_num == 0)
			{
				req_tree* temp = this->left_subtree;
				this->info = temp->info;
				this->left_subtree = temp->left_subtree;
				this->right_subtree = temp->right_subtree;
				delete(temp);
			...
			{
				req_tree* temp = this->right_subtree;
				while (temp->left_subtree->info->flight_num != 0)
	...
	void search_reqs(int flight_num, std::string flight_date)
	{
		if (this->info->flight_num == 0 && this->left_subtree == nullptr && this->right_subtree == nullptr) return;
		if (flight_num > this->info->flight_num)
			this->right_subtree->search_reqs(flight_num, flight_date);
		else
			if (flight_num < this->info->flight_num)
				this->left_subtree->search_reqs(flight_num, flight_date);
			else
			{
				if (flight_date == this->info->flight_date)
				{
					...
					std::cout << "\n\n";
					this->delete_req();
					this->search_reqs(flight_num, flight_date);
				}
				else
